...
def tee_pipe(pipe_stream, first_output, second_output):
    ...
    if available:
        if available > 1024:
            available = 1024 # Keep it responsive
...
def tee_process(process, stdout_file=None, stderr_file=None):
    """
    ...
    with the paramaters stdout=subprocess.PIPE and stderr=subprocess.PIPE and 
    sends the output of stdout and stderr to the specified file-like objects 
    AND to the sys.stdout and sys.stderr endpoints.
    """
    output_value = None
